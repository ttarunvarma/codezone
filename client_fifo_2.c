...
uchar open_fifo (char *fifo, uchar mode){
	...
	_PP(Opening %s\n,fifo);
	fd = open (fifo, mode);
	if(fd == -1) error_call("Client<2> Open Fail");
...
void read_fifo(uchar fd, char *buf, uchar size){
	...
	if(count == -1) error_call("Client<2> Read Fail!");
	else _PP(READ %d bytes from Client<2>FIFO,count);
	buf[count] = '\0';
}
void write_fifo(uchar fd, mydata *buf, uchar size){
	uchar count;
	count =	write(fd, buf, size);
	if(count == -1) error_call("Client<2> Write Fail!");
...
int main()
{
	...
	md.pid = getpid();
	strcpy(md.data,"MSG DATA::Client<2> MESSAGE");
	...
	if(access(cli_fifo, F_OK) == -1)
        {
                if(mkfifo(cli_fifo, 0777)) error_call("Client<2>FIFO creation failed");
                else _PP(Client<2>FIFO Creation Sucessfull);
        }
